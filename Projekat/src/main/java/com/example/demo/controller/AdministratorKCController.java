...
//		Set<Pacijent> listaz = kc.getZahteviZaRegistraciju();
		...
		String text = "Postovani " + pDTO.getIme() + " " + pDTO.getPrezime()
				+ ",\n\nMolimo Vas da potvrdite vasu registraciju klikom na sledeci link: http://localhost:3000/potvrdaRegistracije/"
				+ p.getId() + " .";
	...
	@CrossOrigin(origins = "http://localhost:3000")
	@PreAuthorize("hasAuthority('ADMIN_KC')")
	public ResponseEntity<String> odbijanjeRegistracijePacijenata(@RequestBody PacijentDTO paDTO,
			@PathVariable String razlog) {
		System.out.println("------------------------------------");
		Pacijent p = pacijentService.findByEmail(paDTO.getEmail());
		PacijentDTO pDTO = new PacijentDTO(p);
	...
	public ResponseEntity<?> promeniLozinku(@RequestBody PasswordChanger passCh, Principal pr) {
		...
		if (authenticationManager != null) {
			...
			final Authentication authentication = authenticationManager
					.authenticate(new UsernamePasswordAuthenticationToken(username, passCh.oldPassword));
